...
import useEntityMedia from "@/lib/hooks/useEntityMedia";
...
interface MediaModalProps {
  ...
  entityId: number;
  entityName: string;
  // foreignKey removed - auto-computed in hook
}
...
function MediaModal({
  ...
  useEffect(() => {
    if (isOpen) {
      console.log("🎬 MediaModal Props:", {
        entityType,
        entityId,
        typeofEntityId: typeof entityId,
      });
    }
  }, [isOpen, entityType, entityId]);

  // Use the hook - foreignKey is auto-computed from entityType
  const {
    media,
    images,
    videos,
  ...
  const handleSetPrimary = async (mediaId: number) => {
    const result = await setPrimary(mediaId);
    if (!result.success) {
      alert(result.error);
    }
  };
  ...
  const handleAddMedia = async (e: React.FormEvent) => {
    ...
    // Fixed: entityType is NOT passed here - it's already in the hook
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                {!img.isPrimary && (
                                  <button
                                    ...
                                    className="p-1.5 bg-amber-500 rounded-lg hover:bg-amber-600 transition-colors"
                          ...
                          {img.isPrimary && (
                            <div
                              className="absolute top-2 left-2 px-2 py-0.5 bg-purple-600 text-white 
                                          text-[10px] font-bold rounded-full shadow flex items-center gap-1"
                            >
